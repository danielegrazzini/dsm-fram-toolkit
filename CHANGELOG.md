...
All notable changes to `dsm-fram-toolkit` are documented here.
...
### Added
- Project export now records the fidelity toggle state (`useComono`, `useFullRadius`),
  making result exports self-contained (issue #4).
### Changed
- Export button labels clarified: **"Export JSON (results)"** vs **"Toolkit JSON (data only)"**,
  resolving the ambiguity surfaced by the dual-channel verification campaign (S13).
### Verified
- Math core byte-identical to v7.2; `be_published_replication.js` re-passed (8/8).

## [FEBS Simulator v6.0.2] — 2026-07-19
### Fixed
- Root cause of the `0.1667` weight convention (dual-channel finding, S13): the UI weight
  fields **defaulted** to `0.1667`. Defaults corrected to `0.166667`, `step="any"` (issue #5).
### Added
- **"UNIFORM 1/6"** preset button in the weights panel (issue #5).
- Self-contained results export (issue #4): `sim_stats` now includes `sigma_S`, `sigma_C`,
  `P_S`, `P_C`, the full parameter set (`p`, `beta`, `Pbase`, `rho`, `rmax`, `weights`),
  `theta`, and the active-pair count.
### Verified
- Math core byte-identical to v6; `febs_be_reduction_equivalence.js` re-passed (4/4).
...
### Verified
- Math core byte-identical to v6; `febs_be_reduction_equivalence.js` re-passed (strong
  equivalence 4/4).
...
## [FRAME-Q Simulator — S29] — 2026-07-17
