...
**Verification.** The Python script `tests/sobol_saltelli_verification.py` provides an independent verification of the FRAME-Q Saltelli estimator. It computes first-order and total-order Sobol' indices for the Ishigami function [@ishigami1990], for which all indices have known analytical solutions, and confirms that the estimator recovers those values within Monte Carlo sampling error across Ne ∈ {512, 1024, 2048, 4096}. The script is fully reproducible with a fixed random seed.

# Research Impact Statement

The toolkit has been used in two peer-reviewed contributions. The B-E Simulator and FEBS Simulator supported the analysis presented in @grazzini2024, which developed a methodology for comparing industrial processes using DSM applied to the SeatBridge automotive patent case. The FEBS Simulator was central to @grazzini2026febs, presented at the 28th International DSM Conference (Milan, 2026), which introduced the FEBS framework and demonstrated the silent multiplier phenomenon on the UCAV process design case of @browning2002. The FRAME-Q Simulator underpins the working paper @grazzini2026frameq (in preparation with T.R. Browning and P. Pourghadiri), which presents the full FRAME-Q quantitative framework and global sensitivity analysis results.
